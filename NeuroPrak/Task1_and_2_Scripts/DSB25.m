...
if strcmp(cmd,'process')
    ...
    % GSLC
    [output OA] = GSLC_process_2ChansDelayed(output_delayed,state.OA,param(2),delay,state.d,state.fs,state.c);
    state.OA = OA
    
    output(:,2) = output(:,1);
%     output_red = output_delayed(:,1)*(1/state.nInC)+output_delayed(:,2)*(1/state.nInC);
%     output_red(:,2) = output_red(:,1);
...
elseif strcmp(cmd,'init')
    ...
    state.OA = zeros(input.pagesize,1)
