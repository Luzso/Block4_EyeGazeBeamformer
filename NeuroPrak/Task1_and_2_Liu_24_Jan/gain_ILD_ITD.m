...
if strcmp(cmd,'process')
    ...
    delay_sample = floor((param(3)+1.5)/1000*44100);
    output_tem = [input(delay_sample+1:end);zeros(delay_sample,1)];%input(1:delay_sample)];
    output = output_tem*10^(param(1)/10)*10^(param(2)/10);
...
elseif strcmp(cmd,'getparamnames')
    ...
    output = {'overallRange', 'ILD Range','ITD Range'};
elseif strcmp(cmd,'getparamranges')
    % return a cell array with parameter ranges
    output = {[-15,15], [-15,15],[-1.5,1.5]}; % overall range & ILD range
end